...
from .example import ExamplesSet
...
def t(spec: ModuleSpec) -> None:
    ...
    id = "".join(paths)
    examples_path = EXAMPLES_DIRECTORY_PATH / "/".join(paths) / f"{id}.json"
    assert examples_path.is_file(), f"No example file: {examples_path}"

    examples_list = json.loads(examples_path.read_text())
    assert len(examples_list) > 0, "No examples"

    examples_set_list = ExamplesSet.listOf(examples_list)
    ...
    for solution_index, solution_file_path in enumerate(solution_file_paths):
        ...
        for examples_index, examples_set in enumerate(examples_set_list):
            solution = module.Solution(*examples_set.init)
            ...
            # 遍历示例
            for example_index, example in enumerate(examples_set.examples):
                input = example.input
